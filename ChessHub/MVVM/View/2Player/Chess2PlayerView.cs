...
namespace ChessClient.MVVM.View._2Player
{
    public partial class Chess2PlayerView : Form
    {
        ...
        private readonly Rectangle[,] _highlights;
        ...
        private void InitializeBoard()
        {
            ...
            for (int file = 0; file < 8; file++)
            {
                for (int rank = 0; rank < 8; rank++)
                {
                    ...
                    _pieces[file, rank].DataBindings.Add("Command", _viewModel, "PieceSelectedCommand", true, DataSourceUpdateMode.OnPropertyChanged);
                    Controls.Add(_pieces[file, rank]);

                    _highlights[file, rank] = new Rectangle();
                    _highlights[file, rank].Size = new Size(tileSize, tileSize);
                    _highlights[file, rank].Location = new Point(size.Width - tileSize * (file + 1), size.Width - tileSize * (rank + 1));
                }
            }
        }
